...
| Tailwind + shadcn/ui components  | M4 shipped hand-written Tailwind (Q-APP-3 left undecided). The UX overhaul (2026-07) adopted **shadcn/ui** (new-york style, Radix primitives, Lucide icons) to tokenize the dark-slate theme and kill ~21 duplicated card/button class strings — Q-APP-3 resolved to shadcn. Components are copy-in sources under `components/ui/`.                          |
